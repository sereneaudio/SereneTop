    ...
    switch ( id )
    {
        ...
        case (EventUsrPowerOff):
            ...
            if ( theSink.PowerOffIsEnabled || (!powerManagerIsChargerConnected() && powerManagerIsVbatCritical()) || powerManagerIsVthmCritical())
            {
                ...
                VolumeSetHfpMicrophoneGain(hfp_invalid_link, VOLUME_MUTE_OFF);
        ...
        case (EventUsrVolumeUp):
            ...
            {                
                ...
                MessageSendLater(&theSink.task, EventSysVolumeAndSourceChangeTimer, 0, D_SEC(theSink.conf1->timeouts.StoreCurrentSinkVolumeAndSourceTimeout_s) );
            }            
            break;
        ...
        case (EventSysVolumeAndSourceChangeTimer):
            MAIN_DEBUG(("EventSysVolumeAndSourceChangeTimer\n"));
       ...
       /* User EQ enable indication */
        ...
        case EventSysCheckAudioAmpDrive:
            ...
            if((!IsAudioBusy()) && (!theSink.routed_audio))
            {
                ...
                PioSetPio ( theSink.conf6->PIOIO.pio_outputs.DeviceAudioActivePIO , pio_drive, FALSE) ;
            }
...
/* when using manual audio source routing, these events can be used to switch
   between audio sources */
       ...
       case EventUsrSelectAudioSourceAnalog:
            /* attempt to switch source, anti-flood control enforced */            
            if(!audioRouteSelectedSource(audio_source_ANALOG))
            {
                /* don't indicate event if not powered on or audio libraries still busy */
                lIndicateEvent = FALSE;
            }
       break;
       ...
       case EventUsrSelectAudioSourceSpdif:
            /* attempt to switch source, anti-flood control enforced */            
            if(!audioRouteSelectedSource(audio_source_SPDIF))
            {
                /* don't indicate event if not powered on or audio libraries still busy */
                lIndicateEvent = FALSE;
            }
       ...
       case EventUsrSelectAudioSourceUSB:
            /* attempt to switch source, anti-flood control enforced */            
            if(!audioRouteSelectedSource(audio_source_USB))
            {
                /* don't indicate event if not powered on or audio libraries still busy */
                lIndicateEvent = FALSE;
            }
       ...
       case EventUsrSelectAudioSourceAG1:
            /* attempt to switch source, anti-flood control enforced */            
            if(!audioRouteSelectedSource(audio_source_AG1))
            {
                /* don't indicate event if not powered on or audio libraries still busy */
                lIndicateEvent = FALSE;
            }
       ...
       case EventUsrSelectAudioSourceAG2:
            /* attempt to switch source, anti-flood control enforced */            
            if(!audioRouteSelectedSource(audio_source_AG2))
            {
                /* don't indicate event if not powered on or audio libraries still busy */
                lIndicateEvent = FALSE;
            }
       ...
       /* manually switch to FM audio source */
       case EventUsrSelectFMAudioSource:
            /* attempt to switch source, anti-flood control enforced */            
            if(!audioRouteSelectedSource(audio_source_FM))
            {
                /* don't indicate event if not powered on or audio libraries still busy */
                lIndicateEvent = FALSE;
            }
       ...
       case EventUsrFindMyPhoneImmAlertMild:
        ...
        case EventUsrAuxOutVolUp:
            MAIN_DEBUG(( "HS : Aux Out Vol Up\n" ));
            VolumeModifyAndUpdateRoutedAudioAuxVolume(increase_volume);
        break;
        
        case EventUsrAuxOutVolDown:
            MAIN_DEBUG(( "HS : Aux Out Vol Down\n" ));
            VolumeModifyAndUpdateRoutedAudioAuxVolume(decrease_volume);
        break;
        
        case EventUsrAuxOutVolMuteOn:
            MAIN_DEBUG(( "HS : Aux Out Vol Mute On\n" ));
            VolumeUpdateMuteState(audio_mute_group_aux, AUDIO_MUTE_ENABLE);
        break;
        
        case EventUsrAuxOutVolMuteOff:
            MAIN_DEBUG(( "HS : Aux Out Vol Mute Off\n" ));
            VolumeUpdateMuteState(audio_mute_group_aux, AUDIO_MUTE_DISABLE);
        break;
        
        case EventUsrAuxOutVolMuteToggle:
            MAIN_DEBUG(( "HS : Aux Out Vol Mute Toggle\n" ));
            VolumeToggleMute(audio_mute_group_aux);
        break;
        
        case EventUsrMainOutVolMuteOn:
            MAIN_DEBUG(( "HS : Main Vol Mute On\n" ));
            VolumeUpdateMuteState(audio_mute_group_main, AUDIO_MUTE_ENABLE);
        break;
        
        case EventUsrMainOutVolMuteOff:
            MAIN_DEBUG(( "HS : Main Vol Mute Off\n" ));
            VolumeUpdateMuteState(audio_mute_group_main, AUDIO_MUTE_DISABLE);
        break;
        
        case EventUsrMainOutVolMuteToggle:
            MAIN_DEBUG(( "HS : Main Vol Mute Toggle\n" ));
    ...
    switch (id)
    {
        case AUDIO_PLUGIN_DSP_IND:
            ...
            if (((AUDIO_PLUGIN_DSP_IND_T*)message)->id == KALIMBA_MSG_SOURCE_CLOCK_MISMATCH_RATE)
            {
                ...
                MAIN_DEBUG(("KALIMBA_MSG_SOURCE_CLOCK_MISMATCH_RATE: %d\n", ((AUDIO_PLUGIN_DSP_IND_T*)message)->value[0]));
            }
